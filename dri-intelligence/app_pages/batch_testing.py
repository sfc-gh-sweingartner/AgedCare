...
- Use Feedback Loop page to analyze rejection patterns and get AI suggestions
    ...
    with tab1:
        ...
        if residents is not None and len(residents) > 0:
            ...
            max_residents = min(len(resident_list), 50)
            
            if max_residents == 1:
                batch_sample_size = 1
                st.info("Only 1 resident available in the system")
            else:
                batch_sample_size = st.slider(
                    "Number of residents to process",
                    min_value=1,
                    max_value=max_residents,
                    value=min(5, max_residents),
                    ...
                    if result:
                        ...
                        if parsed_json:
                            ...
                            analysis_id_result = execute_query(f"""
                                SELECT ANALYSIS_ID FROM DRI_LLM_ANALYSIS 
...
- **<85% approval**: Review rejection reasons for improvements
        ...
        overview_data = execute_query_df("""
            ...
            FROM DRI_LLM_ANALYSIS lla
            LEFT JOIN DRI_REVIEW_QUEUE rq 
    ...
    with tab3:
        ...
        gt_count = execute_query("""
            SELECT COUNT(*) as COUNT FROM DRI_GROUND_TRUTH WHERE IS_ACTIVE = TRUE
        """, session)
        ...
        with col2:
            pending_reviews = execute_query("""
                SELECT COUNT(*) as COUNT FROM DRI_REVIEW_QUEUE WHERE STATUS IN ('APPROVED', 'REJECTED')
            """, session)
            ...
            try:
                result = execute_query("""
                    ...
                    WHERE rq.STATUS IN ('APPROVED', 'REJECTED')
                      ...
                      AND NOT EXISTS (
                          SELECT 1 FROM DRI_GROUND_TRUTH gt 
